...
def row2cell_refill_once(
    model: nn.Module,
    row_col_embedding: torch.Tensor,
    hyper_node: torch.Tensor,
    ve_affiliation: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    # Reuse existing model layers: one extra E2V then one V2E after FK row update.
    # e2v_layers[0] consumes raw cell features (dim=1), then v2e_layers[1] (if exists)
    # refreshes row/col from refilled cells without introducing new parameters.
    cell_refilled = model.e2v_layers[0](row_col_embedding, hyper_node, ve_affiliation)
    v2e_idx = 1 if len(model.v2e_layers) > 1 else 0
    row_col_refreshed = model.v2e_layers[v2e_idx](row_col_embedding, cell_refilled, ve_affiliation)
    return row_col_refreshed, cell_refilled
...
def train_all(
    ...
    for epoch in range(args.epochs):
        ...
        for table_name, table_meta in all_table_meta.items():
            ...
            embedding, _ = row2cell_refill_once(
                model,
                embedding,
                train_hyper_node,
                train_ve_affiliation,

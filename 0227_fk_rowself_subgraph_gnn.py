...
def build_relation_graph(rel: FKRelationData, row_embs: Dict[str, torch.Tensor]) -> RelationGraphData:
    x_src = row_embs[rel.src_table]
    x_tgt = row_embs[rel.tgt_table]
    n_src = x_src.size(0)
    n_tgt = x_tgt.size(0)
    ...
    return RelationGraphData(x=x, edge_index=edge_index, n_src=n_src, n_tgt=n_tgt)
...
def evaluate_relations(
    args,
    relations: List[FKRelationData],
    relation_graphs: Dict[str, RelationGraphData],
    relation_models: nn.ModuleDict,
    ...
    for rel in relations:
        g = relation_graphs[rel.rel_key]
        model = relation_models[rel.rel_key]
        with torch.no_grad():
            z = model(g.x, g.edge_index)

        src = z[rel.src_row_idx_eval]
        tgt_all = z[g.n_src : g.n_src + g.n_tgt]
        scores = src @ tgt_all.t()
        ...
        n = src.size(0)
        per_rel.append((rel.rel_name, top1, recall_k, mrr, map_k, n))
        top1_sum += top1 * n
        topk_sum += recall_k * n
        mrr_sum += mrr * n
        map_sum += map_k * n
...
def train_subgraph_bpr(
    args,
    relations: List[FKRelationData],
    relation_graphs: Dict[str, RelationGraphData],
    relation_models: nn.ModuleDict,
) -> None:
    optimizer = torch.optim.AdamW(relation_models.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    for epoch in range(args.epochs):
        relation_models.train()
        losses = []
        ...
        for rel in relations:
            ...
            for st in range(0, src_all.numel(), batch_size):
                ...
                src = src_all[st:ed]
                pos = pos_tgt_all[st:ed]
                neg = torch.randint(0, g.n_tgt, pos.shape, device=pos.device)
                same = neg == pos
                if torch.any(same):
                    neg[same] = (neg[same] + 1) % g.n_tgt

                seed_nodes = torch.cat([src, pos + g.n_src, neg + g.n_src], dim=0)
                ...
                z_sub = model(g.x[subset], sub_edge_index)
                b = src.size(0)
                src_z = z_sub[mapping[:b]]
                pos_z = z_sub[mapping[b : 2 * b]]
                neg_z = z_sub[mapping[2 * b :]]
        ...
        if epoch % args.log_every == 0:
            relation_models.eval()
            _, agg = evaluate_relations(args, relations, relation_graphs, relation_models)
            train_loss = float(np.mean(losses)) if losses else float("nan")
            print(
                f"[fk_subgraph] epoch {epoch} train_loss={train_loss:.6f} "
...
def main() -> None:
    ...
    # Keep same defaults as 0225 for checkpoint compatibility.
    ...
    parser.add_argument("--impute_dropout", type=float, default=0.0)
    ...
    for table_name, meta in table_meta.items():
        meta["fk_true_row_indices"] = build_fk_true_row_indices(meta["df"], table_meta, meta["fkey_map"])
    ...
    # Pre-num learned row_self as fixed node initial features.
    ...
    relation_models = nn.ModuleDict()
    for rel in relations:
        relation_graphs[rel.rel_key] = build_relation_graph(rel, row_embs)
        relation_models[rel.rel_key] = SubgraphSAGE(
            hidden_dim=args.hyperedge_dim_hidden,
    ...
    for rel in relations:
        ...
        print(
            ...
            f"n_eval={rel.src_row_idx_eval.numel()} n_src={g.n_src} n_tgt={g.n_tgt} "
            f"n_train_edges={g.edge_index.size(1)//2}"
        )

    train_subgraph_bpr(args, relations, relation_graphs, relation_models)

    relation_models.eval()
    per_rel, agg = evaluate_relations(args, relations, relation_graphs, relation_models)

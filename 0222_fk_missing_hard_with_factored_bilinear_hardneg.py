...
class RelationFactoredBilinearScorer(nn.Module):
    def __init__(self, dim: int, mode: str = "diag", rank: int = 16) -> None:
        super().__init__()
        self.mode = mode
        ...
        if mode == "diag":
            ...
            raise ValueError(f"Unsupported bilinear mode: {mode}")

    def score_pair(self, query: torch.Tensor, cand: torch.Tensor) -> torch.Tensor:
        # query: [B, D], cand: [B, D] -> [B, 1]
        if self.mode == "diag":
            return ((query * self.diag) * cand).sum(dim=1, keepdim=True) + self.bias
        qf = query @ self.u
        cf = cand @ self.v
        return (qf * cf).sum(dim=1, keepdim=True) + self.bias

    def score_all(self, query: torch.Tensor, cands: torch.Tensor) -> torch.Tensor:
        # query: [B, D], cands: [N, D] -> [B, N]
        if self.mode == "diag":
            return (query * self.diag) @ cands.t() + self.bias
        qf = query @ self.u
        cf = cands @ self.v
        return qf @ cf.t() + self.bias
...
def compute_fk_matching_loss(
    ...
    for fk_col, target_table in table_meta["fkey_map"].items():
        ...
        scorer: RelationFactoredBilinearScorer = match_heads[fk_col]
        src_emb = row_self[src_idx]
        scores_all = scorer.score_all(src_emb, target_rows)
        pos_scores = scores_all.gather(1, pos_idx.unsqueeze(1))

        num_tgt = target_rows.size(0)
        k = min(neg_k, max(1, num_tgt - 1))

        # At least half negatives are hard negatives from current high-score wrong candidates.
        hard_k = int(round(k * hard_neg_ratio))
        hard_k = max(0, min(k, hard_k))
        if hard_k > 0:
            masked_scores = scores_all.clone()
            row_ids = torch.arange(src_emb.size(0), device=row_self.device)
            masked_scores[row_ids, pos_idx] = -1e9
            hard_k_eff = min(hard_k, max(1, num_tgt - 1))
            hard_idx = torch.topk(masked_scores, k=hard_k_eff, dim=1).indices
        else:
            hard_idx = torch.empty((src_emb.size(0), 0), dtype=torch.long, device=row_self.device)

        rand_k = k - hard_idx.size(1)
        if rand_k > 0:
            rand_idx = torch.randint(0, num_tgt, (src_emb.size(0), rand_k), device=row_self.device)
            if num_tgt > 1:
                pos_expand = pos_idx.unsqueeze(1)
                rand_idx = torch.where(rand_idx == pos_expand, (rand_idx + 1) % num_tgt, rand_idx)
            neg_idx = torch.cat([hard_idx, rand_idx], dim=1)
        else:
            neg_idx = hard_idx

        neg_scores = scores_all.gather(1, neg_idx)
...
def init_models(args, table_meta: Dict, device: torch.device) -> None:
    ...
    for fk_col in table_meta["fkey_map"].keys():
        fk_match_heads[fk_col] = RelationFactoredBilinearScorer(
            args.hyperedge_dim_hidden,
            mode=args.bilinear_mode,
...
def main():
    ...
    parser.add_argument("--bilinear_mode", type=str, default="diag", choices=["diag", "lowrank"])

...
def main():
    ...
    for table_name, meta in table_meta.items():
        ...
        if meta["fkey_map"]:
            for fk_col, idxs in meta["fk_row_indices"].items():
                idxs_arr = np.asarray(idxs)
                valid_ratio = float((idxs_arr >= 0).sum()) / max(1, idxs_arr.shape[0])
                raw_na_ratio = float(pd.isna(meta["df"][fk_col]).sum()) / max(1, len(meta["df"]))
                obs_ratio = float(meta["fk_observed"][fk_col].sum()) / max(1, len(meta["fk_observed"][fk_col]))
                print(
                    f"fk_stats table={table_name} col={fk_col} "
                    f"valid_edge_ratio={valid_ratio:.4f} raw_na_ratio={raw_na_ratio:.4f} "
                    f"observed_ratio={obs_ratio:.4f}"
                )

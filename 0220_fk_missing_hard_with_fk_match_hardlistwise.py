...
def compute_fk_matching_loss(
    ...
    for fk_col, target_table in table_meta["fkey_map"].items():
        ...
        k = min(hard_k, max(1, num_tgt - 1))
        hard_neg_scores = torch.topk(masked_scores, k=k, dim=1).values
...
def evaluate_fk_metrics(
    ...
    for fk_col, target_table in table_meta["fkey_map"].items():
        ...
        for st in range(0, src_emb.size(0), batch_size):
            ...
            true_scores = scores.gather(1, true_b.unsqueeze(1)).squeeze(1)
            rank = (scores > true_scores.unsqueeze(1)).sum(dim=1) + 1
            mrr_sum += (1.0 / rank.float()).sum().item()
            total += src_b.size(0)
            pos_score_sum += true_scores.sum().item()
            if scores.size(1) > 1:
                neg_mean = (scores.sum(dim=1) - true_scores) / float(scores.size(1) - 1)
            else:
                neg_mean = torch.zeros_like(true_scores)
            neg_score_sum += neg_mean.sum().item()

            # Hard negatives: per query top-5 mean over negatives (exclude true target).
            if scores.size(1) > 1:
                neg_scores = scores.clone()
                neg_scores.scatter_(1, true_b.unsqueeze(1), float("-inf"))
                hard_k = min(5, neg_scores.size(1) - 1)
                if hard_k > 0:
                    hard_topk_vals = torch.topk(neg_scores, k=hard_k, dim=1).values
                    hard_neg_mean = hard_topk_vals.mean(dim=1)
                else:
                    hard_neg_mean = torch.zeros_like(true_scores)
            else:
                hard_neg_mean = torch.zeros_like(true_scores)
            hard_neg_score_sum += hard_neg_mean.sum().item()
            diag_total += src_b.size(0)
    ...
    return top1_hit / total, topk_hit / total, mrr_sum / total, total, pos_mean, neg_mean, margin, hard_margin
    ...
    for epoch in range(epochs):
        if epoch % args.log_every == 0:
            ...
            if stage != "pretrain_num":
                ...
                for table_name, meta in all_table_meta.items():
                    f_top1, f_rec, f_mrr, f_n, f_pos, f_neg, f_margin, f_hard_margin = evaluate_fk_table(
                        args, meta, all_table_meta, device
                    )
                    if f_n > 0:

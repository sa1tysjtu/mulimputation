...
LOG_ROOT="$ROOT_DIR/logs/fk_complete_sched"
mkdir -p "$LOG_ROOT"
...
run_exp() {
  ...
  {
    echo "# fk_complete sched ablation"
...
# FK complete (do not mask FK), hard propagation only, no fk loss, scheduler enabled.
run_exp "fk_complete_hard_only_sched" "FK not masked, hard FK propagation only + scheduler" \

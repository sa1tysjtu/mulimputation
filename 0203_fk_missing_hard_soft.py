...
def _soft_align_rows(
    src_rows: torch.Tensor,
    tgt_rows: torch.Tensor,
    pool_size: int,
    top_k: int,
    temperature: float,
) -> torch.Tensor:
    num_tgt = tgt_rows.size(0)
    if num_tgt == 0:
        return torch.zeros_like(src_rows)
    if num_tgt > pool_size:
        idx = torch.randperm(num_tgt, device=tgt_rows.device)[:pool_size]
        cand = tgt_rows[idx]
    else:
        cand = tgt_rows

    sims = src_rows @ cand.t()
    if temperature != 1.0:
...
def apply_fk_hard_soft_propagation(
    ...
    for fk_col, idxs in fk_indices.items():
        ...
        missing_mask = table_meta["fk_missing"].get(fk_col)
        if missing_mask is None:
            continue
        missing_idx = torch.tensor(missing_mask, device=device).nonzero(as_tuple=False).squeeze(-1)
        if missing_idx.numel() == 0:
            continue
        src_rows = row_self[missing_idx]
        soft_rows = _soft_align_rows(src_rows, target_rows, soft_pool, soft_k, soft_temp)
        row_fk[missing_idx] += soft_rows
...
def compute_matching_loss(
    ...
    loss = torch.tensor(0.0, device=row_self.device)
    pair_count = 0
    for fk_col, idxs in table_meta["fk_row_indices"].items():
        target_table = table_meta["fkey_map"].get(fk_col)
        if not target_table:
            continue
        target_rows = other_row_embs.get(target_table)
        if target_rows is None or target_rows.size(0) == 0:
            continue
        idxs_tensor = torch.tensor(idxs, device=row_self.device)
        valid = idxs_tensor >= 0
        if not torch.any(valid):
            continue
        src_idx = valid.nonzero(as_tuple=False).squeeze(-1)
        tgt_idx = idxs_tensor[valid]
        ...
        pos_emb = target_rows[tgt_idx]
        pos_scores = (src_emb * pos_emb).sum(dim=1, keepdim=True)

        num_tgt = target_rows.size(0)
        if num_tgt <= 1:
            continue
        k = min(neg_k, num_tgt - 1)
        neg_idx = torch.randperm(num_tgt, device=row_self.device)[:k]
        neg_emb = target_rows[neg_idx]
        neg_scores = src_emb @ neg_emb.t()
    ...
    for epoch in range(args.epochs):
        if epoch % args.log_every == 0:
            ...
            if total > 0:
                ...
                print(f"  eval rmse={avg_rmse:.4f} mae={avg_mae:.4f}")
        ...
        for table_name, table_meta in all_table_meta.items():
            ...
            match_loss = compute_matching_loss(
                table_meta,
                row_self,
                other_row_embs,
                neg_k=args.match_neg_k,
                max_pairs=args.match_max_pairs,
            )

            embedding = apply_fk_hard_soft_propagation(
                row_self,
                col_self,
                table_meta,
                other_row_embs,
                soft_pool=args.soft_pool,
                soft_k=args.soft_k,
            ...
            loss = huber(pred_train, train_labels) + args.match_weight * match_loss
            loss.backward()
            optimizer.step()
...
def main():
    ...
    parser.add_argument("--soft_pool", type=int, default=2000)
    parser.add_argument("--soft_k", type=int, default=20)
    parser.add_argument("--soft_temp", type=float, default=1.0)
    parser.add_argument("--match_weight", type=float, default=0.1)
    parser.add_argument("--match_neg_k", type=int, default=20)
    parser.add_argument("--match_max_pairs", type=int, default=2000)
    ...
    for table_name, table in tables.items():
        ...
        for fk_col in table.fkey_col_to_pkey_table.keys():
            obs = build_fk_observed_mask(
                table.df[fk_col],
                missing_ratio=args.missing_ratio,
                missing_mechanism=args.missing_mechanism,
                seed=args.seed,
            )
            fk_observed[fk_col] = obs
            fk_missing[fk_col] = ~obs
        table_meta[table_name]["fk_observed"] = fk_observed
        table_meta[table_name]["fk_missing"] = fk_missing

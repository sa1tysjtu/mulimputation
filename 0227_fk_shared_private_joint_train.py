...
def project_row_spaces(table_meta: Dict, row_self: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    row_shared = table_meta["row_proj_shared"](row_self)
    row_num = table_meta["row_proj_num"](row_self)
    row_fk = table_meta["row_proj_fk"](row_self)
    return row_shared, row_num, row_fk


def row_for_num_task(table_meta: Dict, row_self: torch.Tensor) -> torch.Tensor:
    row_shared, row_num, _ = project_row_spaces(table_meta, row_self)
    return row_shared + row_num


def row_for_fk_task(table_meta: Dict, row_self: torch.Tensor) -> torch.Tensor:
    row_shared, _, row_fk = project_row_spaces(table_meta, row_self)
    return row_shared + row_fk


def compute_sp_orth_loss(table_meta: Dict, row_self: torch.Tensor) -> torch.Tensor:
    _, row_num, row_fk = project_row_spaces(table_meta, row_self)
    row_num_n = F.normalize(row_num, p=2, dim=1, eps=1e-8)
    row_fk_n = F.normalize(row_fk, p=2, dim=1, eps=1e-8)
    cos_sq = (row_num_n * row_fk_n).sum(dim=1).pow(2)
    return cos_sq.mean()
...
def configure_stage_optimizers(args, all_table_meta: Dict[str, Dict], stage: str) -> None:
    for table_meta in all_table_meta.values():
        ...
        row_proj_shared = table_meta["row_proj_shared"]
        row_proj_num = table_meta["row_proj_num"]
        row_proj_fk = table_meta["row_proj_fk"]
        ...
        if stage == "joint":
            ...
            set_requires_grad(row_proj_shared, True)
            set_requires_grad(row_proj_num, True)
            set_requires_grad(row_proj_fk, True)
            ...
            params = (
                ...
                + list(row_proj_shared.parameters())
                + list(row_proj_num.parameters())
                + list(row_proj_fk.parameters())
                + list(fk_match_heads.parameters())
                + list(fk_rerank_heads.parameters())
            )
        elif stage == "joint_sp":
            # Jointly optimize shared/num/fk projections each step with
            # L_num + lambda_fk * L_fk + lambda_orth * L_orth, while keeping
            # the backbone fixed for a controlled shared-private ablation.
            ...
            set_requires_grad(row_proj_shared, True)
            set_requires_grad(row_proj_num, True)
            set_requires_grad(row_proj_fk, False)
            ...
            set_requires_grad(impute_model, False)
            set_requires_grad(fk_gate, False)
            set_requires_grad(row_proj_shared, not bool(args.sp_freeze_shared_in_fk))
            set_requires_grad(row_proj_num, False)
            set_requires_grad(row_proj_fk, True)
            set_requires_grad(fk_match_heads, True)
            set_requires_grad(fk_rerank_heads, True)
            params = (
                ([] if bool(args.sp_freeze_shared_in_fk) else list(row_proj_shared.parameters()))
                + list(row_proj_fk.parameters())
                + list(fk_match_heads.parameters())
                + list(fk_rerank_heads.parameters())
            )
        ...
        torch.save(
            {
                ...
                "row_proj_shared": table_meta["row_proj_shared"].state_dict(),
                "row_proj_num": table_meta["row_proj_num"].state_dict(),
                "row_proj_fk": table_meta["row_proj_fk"].state_dict(),
            },
            ckpt_path,
        )
...
def train_all(
    ...
    for epoch in range(epochs):
        ...
        for table_name, table_meta in all_table_meta.items():
            ...
            row_num_self = row_for_num_task(table_meta, row_self)
            row_fk_self = row_for_fk_task(table_meta, row_self)

            other_row_num_embs: Dict[str, torch.Tensor] = {}
            other_row_fk_embs: Dict[str, torch.Tensor] = {}
            for fk_col, target_table in table_meta["fkey_map"].items():
                target_meta = all_table_meta.get(target_table)
                if target_meta is None or target_table in other_row_num_embs:
                    continue
                ...
                other_row_num_embs[target_table] = row_for_num_task(target_meta, tgt_row).detach()
                other_row_fk_embs[target_table] = row_for_fk_task(target_meta, tgt_row).detach()
            ...
            loss_orth = compute_sp_orth_loss(table_meta, row_self)
            ...
            if stage == "joint":
                loss = loss_num + args.fk_match_weight * loss_match + args.sp_orth_weight * loss_orth
            elif stage == "joint_sp":
                loss = loss_num + args.joint_fk_weight * loss_match + args.sp_orth_weight * loss_orth
            elif stage == "pretrain_num":
                loss = loss_num
            elif stage == "finetune_fk":
                loss = loss_match + args.sp_orth_weight * loss_orth
            else:
                raise ValueError(f"Unsupported stage: {stage}")
...
def evaluate_table(
    ...
    with torch.no_grad():
        ...
        for fk_col, target_table in table_meta["fkey_map"].items():
            ...
            other_row_embs[target_table] = row_for_num_task(target_meta, tgt_row)
...
def evaluate_fk_table(
    ...
    with torch.no_grad():
        ...
        for fk_col, target_table in table_meta["fkey_map"].items():
            ...
            other_row_embs[target_table] = row_for_fk_task(target_meta, tgt_row)
        return evaluate_fk_metrics(args, table_meta, row_fk_self, other_row_embs)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="joint_sp",
        choices=["joint", "joint_sp", "pretrain_num", "finetune_fk", "two_stage"],
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="FK loss weight for stage=joint_sp.",
    )
    ...
    parser.add_argument("--sp_orth_weight", type=float, default=1e-3)
    parser.add_argument(
        "--sp_freeze_shared_in_fk",
        type=int,
        default=0,
        help="If 1, freeze row_proj_shared during finetune_fk and train only FK-private branch + FK heads.",
    )
    ...
    if args.stage == "joint":
    ...
    elif args.stage == "joint_sp":
        if args.load_pretrain_ckpt_dir:
        ...
        configure_stage_optimizers(args, table_meta, "joint_sp")
        train_all(args, table_meta, device, stage="joint_sp", epochs=args.epochs)
